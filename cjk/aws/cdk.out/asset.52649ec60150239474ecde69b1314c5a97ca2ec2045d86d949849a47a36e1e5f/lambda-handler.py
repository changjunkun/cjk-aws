...
def aws_ec2(event):
    ...
    if event['detail']['eventName'] == 'CreateVpcEndpoint':
        ...
        if tag_set is not None:
            ...
            print(f"Type of tag_items: {type(tag_items).__name__}")  # 添加调试信息
            if isinstance(tag_items, dict):
                ...
                for tag in tag_items:
                    if isinstance(tag, dict):  # 确保 tag 是字典类型
                        if tag.get('key') == 'ClusterArn':
                            msk_arn = tag.get('value')
                            arnList.append(msk_arn)
                            print(f"Extracted MSK Cluster ARN: {msk_arn}")
                    else:
                        print(f"Unexpected type in tag_items list: {type(tag).__name__}")
            elif isinstance(tag_items, str):
                print("tag_items is a string, unexpected behavior.")
            else:
                print(f"Unexpected tag_items type: {type(tag_items).__name__}")
        else:
            print("'tagSet' key not found in the VPC endpoint details.")
